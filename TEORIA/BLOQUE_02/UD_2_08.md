...
* **sort()** → ordena de menor a mayor (modifica la original).
* **sort(reverse=True)** → ordena de mayor a menor (modifica la original).
...
* **.get(clave, valor_defecto)** → devuelve el valor si existe, o un valor por defecto si no.
* **.setdefault(clave, valor_defecto)** → añade la clave con un valor si no existe.
* **.update({...})** → actualiza varias claves a la vez (y si alguna clave no existe la crea).
