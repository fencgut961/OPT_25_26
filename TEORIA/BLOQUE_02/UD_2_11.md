...
## 4️⃣ Recorrer un iterador usando `next()` con valor por defecto (`None`)

Además de usar un bucle `for`, podemos avanzar manualmente por un iterador usando la función `next()`.  
Una forma práctica y limpia de evitar la excepción `StopIteration` es proporcionar un **valor por defecto**, que se devolverá cuando el iterador se agote.
...
* Si aún quedan elementos, los devuelve normalmente.
...
Esta técnica permite recorrer un iterador de forma controlada sin capturar errores.
